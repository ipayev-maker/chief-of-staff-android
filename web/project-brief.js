(function (root, factory) {
  ...
  function renderOverview(model, state = {}) {
    ...
    const card = (key, placeholder, lead = false) => `<section class="pb-card ${lead ? 'pb-state' : ''}"><span class="pb-caption">${labels[key]}</span><p class="${doc[key] ? 'pb-value' : 'pb-placeholder'}">${escape(doc[key] || placeholder)}</p></section>`;
  ...
  function mount(container, options = {}) {
    ...
    function openEditor(kind = null) {
      ...
      if (!entryId) dialog.querySelector('[data-pb-field="current_state"]')?.focus();
    }
    async function save() {
    ...
    container.onclick = event => {
      const target = event.target.closest('[data-pb-action]');
      if (!target || !container.contains(target) || disposed) return;
